...
pub extern "C" fn _start(argc: usize, argv: usize) -> ! {
    ...
    for idx in 0..argc {
        let str_start = unsafe { ((argv + idx * core::mem::size_of::<usize>()) as *const usize).read_volatile() } as *const u8;
        let str_len = unsafe {
            let mut p = str_start;
            let mut len = 0;
            loop {
        ...
        v.push(core::str::from_utf8(
            unsafe { core::slice::from_raw_parts(str_start, str_len) }
        ).unwrap());
    }
...
pub fn sigaction(
    ...
    sys_sigaction(
        signum,
        action.map_or(core::ptr::null(), |action| action),
        old_action.map_or(core::ptr::null_mut(), |action| action),
    )
}
